...
def init_app():
    ...
    with app.app_context():
        ...
        # Проверка схемы БД (для диагностики проблем)
        try:
            import sqlite3
            ...
            # Проверяем схему таблицы Suggestion
            cursor.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='suggestion'")
            result = cursor.fetchone()

            if result and 'uix_entity_value' not in result[0]:
                print("\n" + "!" * 70)
                print("⚠ ВНИМАНИЕ: Обнаружена устаревшая схема базы данных!")
                print("!" * 70)
                print("Решение: Удалите файл базы данных и перезапустите приложение:")
                print(f"  {db_path}")
                print("После удаления БД будет создана заново с правильной схемой.")
                print("!" * 70 + "\n")

            conn.close()
        except Exception as e:
            print(f"⚠ Не удалось проверить схему БД: {e}")
